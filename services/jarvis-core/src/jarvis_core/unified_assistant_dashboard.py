...
(function() {
    ...
    sessionState.transcript.forEach((turn, index) => {
      ...
      if (turn.role === 'user') {
        ...
        if (turn.error) {
          ...
          if (rawCitations && rawCitations.length) {
            citationsHtml = `
              <div class="citations-card">
                <strong>Model Citation Labels:</strong>
                <div style="margin-top:4px; display:grid; gap:4px; font-size:0.84rem;">
                  ${rawCitations.map(c => {
                    if (typeof c === 'object' && c !== null) {
                      const lbl = c.label || 'Reference';
                      const supp = c.supports || '';
                      return `<div style="display:flex; align-items:baseline; gap:6px;"><span class="pill allowed" style="font-size:0.75rem; padding:1px 6px;">${escapeHtml(lbl)}</span> <span class="muted" style="font-size:0.82rem;">— supports: ${escapeHtml(supp)}</span></div>`;
                    }
                    return `<div style="display:flex; align-items:baseline; gap:6px;"><span class="pill allowed" style="font-size:0.75rem; padding:1px 6px;">Reference</span> <span class="muted" style="font-size:0.82rem;">${escapeHtml(String(c))}</span></div>`;
                  }).join('')}
                </div>
              </div>
            `;
          ...
          turnDiv.innerHTML = `
            ...
            ${(resp.comparisonMatrix || resp.suggestedDirection) ? `
              <div style="margin-bottom:8px;">
                <h5 style="margin:0 0 4px; font-size:0.88rem; color:var(--accent-dark);">Primary Decision Response:</h5>
  ...
  function renderDecisionResultHtml(turn, resp, index) {
    ...
    const isHs = !!(resp.safety && resp.safety.high_stakes) || !!(turn.route && (turn.route.high_stakes || turn.route.is_high_stakes));
