...
def test_dashboard_summary_includes_local_extraction_agent(tmp_path, monkeypatch):
    app_services(tmp_path, monkeypatch)

    summary = app_module.dashboard_summary()
    agent = summary["localExtractionAgent"]

    assert summary["capabilities"]["localExtractionAgent"] == "implemented_local_only"
    assert summary["safety"]["localExtractionAgent"]["externalServices"] is False
    assert agent["agentId"] == "local_extraction_agent"
    assert agent["name"] == "Local Extraction Agent"
    assert agent["status"] == "implemented_local_only"
    assert agent["mode"] == "response_only_user_provided_extraction"
    assert agent["endpoint"] == "/agents/extraction/local-extract"
    assert agent["extractionTypes"] == [
        "general",
        "action_items",
        "requirements",
        "risks",
    ...
    assert agent["extractionPersistence"] is False
...
def test_dashboard_html_includes_read_only_local_extraction_agent_section(tmp_path, monkeypatch):
    app_services(tmp_path, monkeypatch)

    page_text = app_module.local_dashboard().body.decode("utf-8")

    assert 'id="local-extraction-agent"' in page_text
    assert "Local Extraction Agent" in page_text
    assert "Response-only extraction." in page_text
    assert "POST /agents/extraction/local-extract" in page_text
    assert "/docs/local-extraction-agent.md" in page_text
    assert "renderLocalExtractionAgent(summary.localExtractionAgent)" in page_text


def test_dashboard_local_extraction_agent_section_has_no_unsafe_action_buttons(tmp_path, monkeypatch):
    app_services(tmp_path, monkeypatch)

    page_text = app_module.local_dashboard().body.decode("utf-8").lower()
    start = page_text.index('id="local-extraction-agent"')
